...
/* include file control operation lib */
#include <fcntl.h>

/* include Naemon stuff */
#include "naemon.h"
...
static void log_modpd_stats(struct nm_event_execution_properties *evprop);
...
int nebmodule_init(int flags, char *args, nebmodule *handle)
{
	...
	/* set some info - this is completely optional, as Naemon doesn't do anything with this data */
	...
	nm_log(NSLOG_INFO_MESSAGE, "modpd: Copyright © 2017-2020 Christian Zettel (ccztux), all rights reserved, Version: 2.3.1");

	/* log a message to the Naemon log file */
	nm_log(NSLOG_INFO_MESSAGE, "modpd: Starting...");

	/* log a status message in N seconds (how's that for annoying? :-)) */
	schedule_event(log_stats_interval, log_modpd_stats, NULL);
...
int nebmodule_deinit(int flags, int reason)
{
	...
	/* write stats data to the Naemon log file */
	log_modpd_stats;
	...
	nm_log(NSLOG_INFO_MESSAGE, "modpd: The modpd NEB module was running %dd %dh %dm %ds", days, hours, minutes, seconds);
	nm_log(NSLOG_INFO_MESSAGE, "modpd: Bye, bye...\n");

	return NEB_OK;
}


/* gets called every N seconds an event in the scheduling queue */
static void log_modpd_stats(struct nm_event_execution_properties *evprop)
{
	int days, hours, minutes, seconds;
	...
	/* log our uptime to the Naemon log file */
	nm_log(NSLOG_INFO_MESSAGE, "modpd: The modpd NEB module is running %dd %dh %dm %ds", days, hours, minutes, seconds);
	...
	/* log the stats to the Naemon log file */
	nm_log(NSLOG_INFO_MESSAGE, "modpd: *** Stats of processed checks for the last %u seconds: Hosts: %u (OK: %u/NOK: %u), Services: %u (OK: %u/NOK: %u) ***\n", timediff, host_cmds_total_counter, host_cmds_ok_counter, host_cmds_nok_counter, service_cmds_total_counter, service_cmds_ok_counter, service_cmds_nok_counter);
...
/* handle data from Naemon daemon */
	...
	switch(callback_type) {
		case NEBCALLBACK_HOST_CHECK_DATA:
			if ((hostdata = (nebstruct_host_check_data *) data)) {
				...
				if (hostdata->type == NEBTYPE_HOSTCHECK_PROCESSED && host->obsess == 1) {
					strcpy(output, "");
		...
		case NEBCALLBACK_SERVICE_CHECK_DATA:
			if ((servicedata = (nebstruct_service_check_data *) data)) {
				...
				if (servicedata->type == NEBTYPE_SERVICECHECK_PROCESSED && service->obsess == 1) {
					strcpy(output, "");
